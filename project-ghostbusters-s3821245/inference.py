...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    # Initialize variables and edges
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    edges = [(GHOST0, OBS0),(PAC, OBS0),(PAC, OBS1), (GHOST1, OBS1)]

    # Initialize agent positions and observed distances lists
    agentPositions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    observedDistances = list(range(X_RANGE+Y_RANGE-1+MAX_NOISE))

    # Initialize variable domains dictionary with agent positions and observed distances
    variableDomainsDict = {PAC: agentPositions,
                            GHOST0: agentPositions,
                            GHOST1: agentPositions,
                            OBS0:observedDistances,
                            OBS1:observedDistances}
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Initialize current factors list with all CPTs with evidence from the bayes net
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # Looping through all elimination variables in the elimination order
        for eliminationVariable in eliminationOrder:
            # Joining all factors by the current elimination variable
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, eliminationVariable)
            # If the joined factor has more than one unconditioned variable, append the joined factor to the current factors list
            if len(joinedFactor.unconditionedVariables())>1:
                # Appending the joined factor to the current factors list
                currentFactorsList.append(eliminate(joinedFactor, eliminationVariable))

        # Joining all factors in the current factors list
        fullJoint = joinFactors(currentFactorsList)
        # Normalizing the full joint
        queryConditionedOnEvidence = normalize(fullJoint)
        # Returning a factor that is conditioned on the evidence variables
        return queryConditionedOnEvidence
